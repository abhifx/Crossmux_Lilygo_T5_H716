...
void beginI2C() {
  ...
  int sda = (touch.sda != BoardConfig::PIN_UNASSIGNED) ? touch.sda : T5S3_SDA;
  int scl = (touch.scl != BoardConfig::PIN_UNASSIGNED) ? touch.scl : T5S3_SCL;
  Wire.begin(sda, scl);
...
void prepareSdBus() {
  ...
  int cs = (sd.cs != BoardConfig::PIN_UNASSIGNED) ? sd.cs : T5S3_SD_CS;
  int sclk = (sd.sclk != BoardConfig::PIN_UNASSIGNED) ? sd.sclk : T5S3_SPI_SCLK;
  int miso = (sd.miso != BoardConfig::PIN_UNASSIGNED) ? sd.miso : T5S3_SPI_MISO;
  int mosi = (sd.mosi != BoardConfig::PIN_UNASSIGNED) ? sd.mosi : T5S3_SPI_MOSI;
...
void begin() {
  ...
  if (BoardConfig::ACTIVE.input.power != BoardConfig::PIN_UNASSIGNED) {
    pinMode(BoardConfig::ACTIVE.input.power, INPUT_PULLUP);
  }
